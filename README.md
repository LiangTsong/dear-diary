...
## ok首页个人信息（127.0.0.1:8000/api/user_info/）
...
## ok首页信息流（127.0.0.1:8000/api/message/）
...
## ok首页提醒事项请求（127.0.0.1:8000/api/notice/）
...
## ok书写页新建日记（127.0.0.1:8000/api/new_diary/）
...
## 书写页特权新建日记（127.0.0.1:8000/api/new_diary_presentation/）

这是展示用接口。POST加入时间。使用此接口新建的日记，请后续在提交时立即进行情绪分析（可以判断日记id是不是通过此接口创建的id，如果是就立即处理）。除此之外，其他表现与正常日记无异。
...
{
    "success": "服务器处理情况Integer,//1表示成功，0表示服务器操作失败",
    "type": "新建情况Integer,//1表示新建成功，并返回空的object_text；0表示新建失败，返回当天已经写过的日记",
    "object_text": "日记内容富文本String",
    "id": "日记唯一编号",
}
```
...
{
    "success": 1,
    "type": "新建情况Integer,//1表示新建成功，并返回空的object_text
    "object_text": "日记内容富文本String",
    "id": 007,
}
```
...
## ok书写页日记提交（127.0.0.1:8000/api/submit_diary/）
...
## ok书写页提醒事项提交(127.0.0.1:8000/api/submit_notice/）
